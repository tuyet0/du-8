...
const ImageUpload = (props) => {
  ...
  return (
    <label
      className={`${className} cursor-pointer flex items-center justify-center border border-dashed w-full h-[300px] relative overflow-hidden group`}
    >
      <input
        type="file"
        name={name}
      ...
      {/* no choose image and progress = 0, no any handle */}
      {!image && progress === 0 && (
      ...
      {/* user choose image, view show image just choose  */}
      {image && (
        <Fragment>
      ...
      {/* image is uploading and the progress will show */}
      {!image && (
        <div
      ...
      {/* image is uploading and the overlay will show */}
      {!image && (
        <>
          <div
            className="bg-[#1c0a4d] absolute inset-0 w-full h-full transition-all opacity-0 z-9"
            style={{
